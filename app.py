...
@st.cache_resource
def train_model(data, model_name="Logistic Regression", test_size=0.20, random_state=42):
    """Huấn luyện Logistic Regression hoặc XGBoost."""
    X = data[FEATURES]
    y = data[TARGET]
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 🤖 Lựa chọn mô hình")
...
model, X_train, X_test, y_train, y_test = train_model(
    data, model_name=model_name, test_size=test_size, random_state=random_state
)

if model_name == "Logistic Regression":
    logistic_step = model.named_steps["model"]
...
if model_name == "Logistic Regression":
    coef_df = pd.DataFrame({
        ...
        "Tác động đến gian lận": [
            "Tăng log-odds" if b > 0 else "Giảm log-odds" if b < 0 else "Không đổi"
            for b in coef
        ]
    })
    interpretation_list = []
    for name, b in zip(FEATURES, coef):
        ...
        interpretation_list.append({
            ...
            "Odds Ratio": or_val,
            "Diễn giải ý nghĩa": (
                f"Khi {name} tăng 1 độ lệch chuẩn, log-odds gian lận "
                f"{'tăng' if b > 0 else 'giảm' if b < 0 else 'không đổi'} "
                f"{abs(b):.4f}; Odds Ratio = {or_val:.4f}, giữ các biến khác không đổi."
            )
        })
    ...
    interpretation_df["Diễn giải ý nghĩa"] = (
        "Mức độ đóng góp tương đối của biến trong mô hình XGBoost."
    )
...
# ==============================================================================
with tab_eval:
    st.subheader(f"🎯 Hiệu Năng Mô Hình {model_name} trên Tập Test")
    ...
    with col_coef:
        if model_name == "Logistic Regression":
            st.markdown("##### 📈 Hệ số Beta & Odds Ratio")
            plot_df = coef_df.sort_values("Hệ số (Beta)")
            fig_coef = px.bar(
                plot_df, x="Hệ số (Beta)", y="Chỉ số", orientation="h",
                color="Hệ số (Beta)", color_continuous_scale="spectral",
                title="Hệ số Beta chuẩn hóa"
            )
        else:
            st.markdown("##### 📈 Feature Importance của XGBoost")
            plot_df = coef_df.sort_values("Feature Importance")
            fig_coef = px.bar(
                plot_df, x="Feature Importance", y="Chỉ số", orientation="h",
                color="Feature Importance", color_continuous_scale="spectral",
                title="Mức độ quan trọng của 8 biến M-Score"
            )
    ...
    if model_name == "Logistic Regression":
        ...
        st.info(
            "XGBoost là mô hình cây tăng cường nên không có phương trình tuyến tính "
            "và hệ số Beta như Logistic Regression. Feature Importance thể hiện "
            "mức độ đóng góp tương đối của từng biến."
        )
    ...
    model_info_df = pd.DataFrame({
        "Thông số": ["Model", "Intercept", "Threshold", "Train_size", "Test_size", "Random_state"],
        "Giá trị": [model_name, intercept if model_name == "Logistic Regression" else "N/A",
                    threshold, len(X_train), len(X_test), random_state]
    })
